...
print $out_fd "\n";
print $out_fd "address:$address\n";
print $out_fd "action:$b58action\n";

print $out_fd "No input!" and exit if length($address) == 0;
print $out_fd "Too many characters! Try the Perl source code below." and exit if length($address) > 20000;

if ($b58action eq "validate"){
	my $address_type = check_bitcoin_address($address);
	print $out_fd "Bitcoin address is valid.  Address type: '", $address_type, "'.\n";
}
elsif ($b58action eq "decode"){
	print $out_fd decodebase58tohex($address);
}
elsif ($b58action eq "encode"){
	print $out_fd encodebase58fromhex($address);
}
else{
	print $out_fd "Invalid base58er action!\n";
}
close $out_fd;
